 ...
 *
 * All detail colors are theme-driven via IslandTheme properties.
 */

import { useMemo } from 'react';
import { getIslandTheme, type IslandTheme } from '@/data/IslandThemes';
...
// ─── Pebbles ──────────────────────────────────────────────────
interface Pebble { cx: number; cy: number; rx: number; ry: number; fill: string; }

function generatePebbles(
  tl: Pt, tr: Pt, bl: Pt, br: Pt,
  bandStart: number, bandEnd: number,
  count: number, seed: number, tint: number,
): Pebble[] {
  ...
  for (let i = 0; i < count; i++) {
    ...
    const shade = Math.floor(tint + seeded(seed + i * 23) * 40);
    pebbles.push({
      cx: pt.x, cy: pt.y,
...
function smoothDiamond(): string {
  ...
  return [
    ...
    `L ${p(botIn)}`, `Q ${p(BOTTOM)} ${p(botOut)}`,
    `L ${p(leftIn)}`, `Q ${p(LEFT)} ${p(leftOut)}`,
    `L ${p(topIn)}`, `Q ${p(TOP)} ${p(topOut)}`, `Z`,
  ].join(' ');
}
...
function generateMossSpots(
  tl: Pt, tr: Pt, bl: Pt, br: Pt, bandStart: number, seed: number,
) {
...
export const IslandSVG = ({ gridSize = 20, themeId = 'day' }: IslandSVGProps) => {
  ...
  // Theme-driven stone blocks (memoized per theme)
  const leftStoneBlocks = useMemo(() =>
    generateStoneBlocks(LW.tl, LW.tr, LW.bl, LW.br, 0.5, 1.0, 6, 5, 42, theme.stoneColors.left),
    [theme.stoneColors.left]
  );
  const rightStoneBlocks = useMemo(() =>
    generateStoneBlocks(RW.tl, RW.tr, RW.bl, RW.br, 0.5, 1.0, 6, 5, 99, theme.stoneColors.right),
    [theme.stoneColors.right]
  );

  // Theme-driven pebbles (memoized per theme)
  const leftPebbles = useMemo(() =>
    generatePebbles(LW.tl, LW.tr, LW.bl, LW.br, 0.12, 0.5, 5, 55, theme.cliffDetails.pebbleTint),
    [theme.cliffDetails.pebbleTint]
  );
  const rightPebbles = useMemo(() =>
    generatePebbles(RW.tl, RW.tr, RW.bl, RW.br, 0.12, 0.5, 5, 77, theme.cliffDetails.pebbleTint),
    [theme.cliffDetails.pebbleTint]
  );
  ...
  >
    <defs>
      ...
      {/* Depth shading — themed */}
      <radialGradient id="ig-shadow" cx="35%" cy="65%" r="55%">
        <stop offset="60%" stopColor="transparent" />
        <stop offset="100%" stopColor={theme.depthShading.shadow} />
      </radialGradient>
      <radialGradient id="ig-sun" cx="60%" cy="32%" r="42%">
        <stop offset="0%" stopColor={theme.depthShading.highlight} />
    ...
    <g clipPath="url(#ig-left-cliff-clip)">
      <polygon points={bandPolygon(LW.tl, LW.tr, LW.bl, LW.br, 0.5, 1)} fill={theme.stoneBaseLeft} />
      ...
      {leftMoss.map((m, i) => (
        <ellipse key={`lms-${i}`} cx={m.cx} cy={m.cy} rx={m.rx} ry={m.ry} fill={theme.cliffDetails.moss} />
      ))}
      <polygon points={bandPolygon(LW.tl, LW.tr, LW.bl, LW.br, 0.12, 0.5)} fill="url(#ig-ld)" />
      {leftDirtStrata.map((ln, i) => (
        <line key={`lds-${i}`} x1={ln.x1} y1={ln.y1} x2={ln.x2} y2={ln.y2} stroke={`rgba(70,50,25,${ln.opacity})`} strokeWidth={0.8} />
      ))}
      {leftPebbles.map((pb, i) => (
        <ellipse key={`lpb-${i}`} cx={pb.cx} cy={pb.cy} rx={pb.rx} ry={pb.ry} fill={pb.fill} stroke="rgba(50,40,30,0.15)" strokeWidth={0.4} />
      ))}
      {leftRoots.map((d, i) => (
        <path key={`lrt-${i}`} d={d} fill="none" stroke={theme.cliffDetails.roots} strokeWidth={0.7} strokeLinecap="round" />
      ))}
      <polygon points={bandPolygon(LW.tl, LW.tr, LW.bl, LW.br, 0, 0.12)} fill="url(#ig-ll-blend)" />
      {leftGrassStrands.map((d, i) => (
        <path key={`lgs-${i}`} d={d} fill="none" stroke={theme.cliffDetails.grassStrands} strokeWidth={1} strokeLinecap="round" />
      ))}
      {(() => {
        const l = lerp(LW.tl, LW.bl, 0.5);
        const r = lerp(LW.tr, LW.br, 0.5);
        return <line x1={l.x} y1={l.y} x2={r.x} y2={r.y} stroke="rgba(25,20,10,0.2)" strokeWidth={0.8} />;
      })()}
    </g>

    {/* Right cliff */}
    <g clipPath="url(#ig-right-cliff-clip)">
      <polygon points={bandPolygon(RW.tl, RW.tr, RW.bl, RW.br, 0.5, 1)} fill={theme.stoneBaseRight} />
      ...
      {rightMoss.map((m, i) => (
        <ellipse key={`rms-${i}`} cx={m.cx} cy={m.cy} rx={m.rx} ry={m.ry} fill={theme.cliffDetails.moss} />
      ))}
      <polygon points={bandPolygon(RW.tl, RW.tr, RW.bl, RW.br, 0.12, 0.5)} fill="url(#ig-rd)" />
      {rightDirtStrata.map((ln, i) => (
        <line key={`rds-${i}`} x1={ln.x1} y1={ln.y1} x2={ln.x2} y2={ln.y2} stroke={`rgba(70,50,25,${ln.opacity})`} strokeWidth={0.8} />
      ))}
      {rightPebbles.map((pb, i) => (
        <ellipse key={`rpb-${i}`} cx={pb.cx} cy={pb.cy} rx={pb.rx} ry={pb.ry} fill={pb.fill} stroke="rgba(50,40,30,0.15)" strokeWidth={0.4} />
      ))}
      {rightRoots.map((d, i) => (
        <path key={`rrt-${i}`} d={d} fill="none" stroke={theme.cliffDetails.roots} strokeWidth={0.7} strokeLinecap="round" />
      ))}
      <polygon points={bandPolygon(RW.tl, RW.tr, RW.bl, RW.br, 0, 0.12)} fill="url(#ig-rl-blend)" />
      {rightGrassStrands.map((d, i) => (
        <path key={`rgs-${i}`} d={d} fill="none" stroke={theme.cliffDetails.grassStrands} strokeWidth={1} strokeLinecap="round" />
      ))}
      {(() => {
        const l = lerp(RW.tl, RW.bl, 0.5);
        const r = lerp(RW.tr, RW.br, 0.5);
        return <line x1={l.x} y1={l.y} x2={r.x} y2={r.y} stroke="rgba(25,20,10,0.18)" strokeWidth={0.8} />;
      })()}
    </g>

    {/* ═══ GRASS OVERHANG ═══ */}
    <path d={GRASS_OVERHANG_PATH} fill="url(#ig-edge)" />
    <path d={GRASS_OVERHANG_PATH} fill="none" stroke={theme.surfaceDetails.overhangStroke} strokeWidth={0.6} />

    {[...leftBlades, ...rightBlades].map((d, i) => (
      <path key={`bl-${i}`} d={d} fill={theme.surfaceDetails.blades} />
    ))}
    ...
    {tiles.map((tile, i) => (
      <polygon key={`t-${i}`} points={tile.points}
        ...
        stroke={theme.surfaceDetails.tileStroke}
        strokeWidth={0.6} />
    ))}
    ...
    <g clipPath="url(#ig-diamond-clip)">
      {grassTufts.map((tuft, i) => (
        <path key={`gt-${i}`} d={tuft.path} fill={theme.surfaceDetails.tufts} />
      ))}
      {sunDapples.map((d, i) => (
        <ellipse key={`sd2-${i}`} cx={d.cx} cy={d.cy} rx={d.rx} ry={d.ry} fill={theme.surfaceDetails.dapples} />
      ))}
      {dirtPatches.map((dp, i) => (
        <ellipse key={`dp-${i}`} cx={dp.cx} cy={dp.cy} rx={dp.rx} ry={dp.ry} fill={theme.surfaceDetails.dirtPatches} />
      ))}
    </g>

    {/* ═══ TIER DECORATIONS — themed ═══ */}
    <g clipPath="url(#ig-diamond-clip)" opacity={0.7}>
      {gridSize >= 7 && <>
        {Array.from({ length: Math.min(gridSize - 4, 12) }, (_, i) => {
          ...
          return (
            <g key={`deco-${i}`}>
              <circle cx={edge.x} cy={edge.y - 2} r={2} fill={theme.tierDecorations.flowerColors[i % theme.tierDecorations.flowerColors.length]} opacity={0.6} />
        ...
        {Array.from({ length: Math.min(gridSize - 8, 6) }, (_, i) => {
          ...
          return (
            <ellipse key={`rock-${i}`}
              ...
              fill={theme.tierDecorations.rockColor} />
          );
        })}
      </>}
      {gridSize >= 14 && <>
        {Array.from({ length: Math.min(gridSize - 12, 4) }, (_, i) => {
          ...
          return (
            <g key={`mush-${i}`}>
              <rect x={pos.x - 0.5} y={pos.y - 1} width={1} height={3} fill={theme.tierDecorations.mushroomStem} rx={0.3} />
              <ellipse cx={pos.x} cy={pos.y - 1.5} rx={2.5} ry={1.5} fill={theme.tierDecorations.mushroomCap} />
            </g>
          );
        })}
      </>}
    ...
    <path d={DIAMOND_SMOOTH} fill="none" stroke={theme.surfaceDetails.overhangStroke} strokeWidth={1} strokeLinejoin="round" />
